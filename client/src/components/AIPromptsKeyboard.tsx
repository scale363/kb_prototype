...
export function AIPromptsKeyboard({ text, selectedText, previewText, onPreviewTextChange, onTextChange, onSwitchKeyboard }: AIPromptsKeyboardProps) {
  ...
  const handleToneSelect = (toneId: string) => {
    ...
    const newResult: RephraseResult = {
      id: `result-${Date.now()}`,
      text: `[${toneName} - ${langName}] This is a rephrased placeholder text for testing.`,
      tone: toneId,
  ...
  const handleReprocess = () => {
    ...
    const newResult: RephraseResult = {
      id: `result-${Date.now()}`,
      text: `[${toneName} - ${langName}] Reprocessed text iteration #${rephraseResults.length + 1}.`,
      tone: selectedTone,
      language: selectedLanguage,
      timestamp: Date.now(),
    };
  ...
  const handlePromptClick = async (promptId: string) => {
    switch (promptId) {
      case "rephrase":
        ...
        break;
      case "translate":
        toast({ title: "Translate", description: "Available soon" });
        break;
      case "snippets":
        toast({ title: "Snippets", description: "Available soon" });
        break;
      case "clipboard":
        try {
          const clipboardText = await navigator.clipboard.readText();
        ...
        {TONE_OPTIONS.map((tone) => (
          <div key={tone.id} className="relative">
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 min-h-[56px] p-3 rounded-xl border-2 active:scale-[0.98] transition-transform duration-75 touch-manipulation ${tone.colorClass} ${tone.borderClass}`}
            >
  ...
  const renderResult = () => (
    <div className="flex flex-col gap-3 p-1 max-h-[400px]">
        ...
        {rephraseResults.map((result, index) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col gap-2 p-4 rounded-xl cursor-pointer ${isSelected ? "bg-accent/50 border-2 border-primary" : "bg-accent/30 border-2 border-accent"} active:scale-[0.99] transition-all duration-75`}
            >
              ...
              {isSelected && (
                <div className="flex gap-2 pt-2">
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); handleCopyResult(result.id); }}
                    className="flex-1 flex items-center justify-center gap-2 min-h-[40px] px-3 rounded-lg border-2 bg-secondary border-border active:scale-[0.98] transition-all duration-75"
                  >
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); handleApplyResult(result.id); }}
                    className="flex-1 flex items-center justify-center gap-2 min-h-[40px] px-3 rounded-lg border-2 bg-primary border-primary-border text-primary-foreground active:scale-[0.98] transition-all duration-75"
                  >
      ...
      <div className="flex gap-2 pt-2 border-t border-border mt-auto">
        <button onClick={handleReprocess} className="flex-1 flex items-center justify-center gap-2 min-h-[44px] rounded-xl border-2 border-border bg-card">
          <RotateCcw className="h-4 w-4" />
          <span className="text-sm font-medium">Reprocess</span>
        </button>
        <button onClick={handleBackToTones} className="flex items-center justify-center min-h-[44px] px-4 rounded-xl border-2 border-border bg-card">
  ...
  const renderMainMenu = () => {
    ...
    const PROMPT_BUTTONS_CONFIG = [
      { id: "rephrase", label: "Rephrase", icon: <RefreshCw className="h-6 w-6 stroke-[2.5px]" />, colorClass: "hover:bg-blue-50 hover:border-[#3b82f6] text-[#3b82f6]" },
      { id: "translate", label: "Translate", icon: <Languages className="h-6 w-6 stroke-[2.5px]" />, colorClass: "hover:bg-purple-50 hover:border-[#8b5cf6] text-[#8b5cf6]" },
      { id: "snippets", label: "Snippets", icon: <FileText className="h-6 w-6 stroke-[2.5px]" />, colorClass: "hover:bg-green-50 hover:border-[#10b981] text-[#10b981]" },
      { id: "clipboard", label: "Clipboard", icon: <Clipboard className="h-6 w-6 stroke-[2.5px]" />, colorClass: "hover:bg-orange-50 hover:border-[#f59e0b] text-[#f59e0b]" },
    ];

    return (
      <div className="grid grid-cols-2 gap-3 p-1">
        {PROMPT_BUTTONS_CONFIG.map((button) => (
          <button
            key={button.id}
            type="button"
            onClick={() => handlePromptClick(button.id)}
            className={`flex flex-col items-center justify-center p-6 rounded-2xl border-2 border-border bg-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 ${button.colorClass}`}
          >
            <div className="mb-3">{button.icon}</div>
            <span className="text-[15px] font-semibold text-foreground">{button.label}</span>
          </button>
        ))}
      </div>
    );
